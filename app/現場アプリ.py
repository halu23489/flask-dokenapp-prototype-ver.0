...
class LevelSurveyNotebook:
    def __init__(self, parent):
        ...
        self.calculate_button = ttk.Button(parent, text="計算", command=self.calculate_elevation)
        self.calculate_button.pack(pady=5)

        self.add_button = ttk.Button(parent, text="測点を追加", command=self.add_point)
        self.add_button.pack(pady=5)

        self.set_bm_button = ttk.Button(parent, text="BMに設定", command=self.set_benchmark)
        self.set_bm_button.pack(pady=5)

        self.unset_bm_button = ttk.Button(parent, text="BMを解除", command=self.unset_benchmark)
        self.unset_bm_button.pack(pady=5)

        self.save_dwg_button = ttk.Button(parent, text="DXFに保存", command=self.save_to_dwg)
...
# 計算ボタン
calculate_button = ttk.Button(tab1, text="計算", command=calculate_volume_weight)
calculate_button.grid(column=0, row=3, columnspan=2, padx=10, pady=10)

# コピーするボタン
copy_button = ttk.Button(tab1, text="コピー", command=copy_to_clipboard)
...
# 計算ボタン
calc_button = ttk.Button(tab2, text="計算", command=calculate_cutting_load)
...
# HEIF変換のボタン
convert_button = tk.Button(tab4, text="HEIFファイルを選択", command=select_heif_files)
